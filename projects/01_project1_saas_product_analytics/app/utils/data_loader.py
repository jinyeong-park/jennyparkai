...
def load_usage(data_dir: Path | None = None) -> pd.DataFrame:
    """Load the weekly usage table (synthetic, with a planted signal; see the generator)."""
    source = data_dir or DEFAULT_DATA_DIR
    return pd.read_csv(source / "usage_weekly.csv", parse_dates=["week_start"])
